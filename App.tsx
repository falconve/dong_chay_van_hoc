...
export default function App() {
  ...
  const resetLeaderboard = useCallback(async () => {
    if (
      !window.confirm(
        "BẠN CÓ CHẮC CHẮN MUỐN XÓA TẤT CẢ KẾT QUẢ?\nDữ liệu trên Google Sheet sẽ bị xóa vĩnh viễn!",
      )
    )
      return;
      ...
      <AnimatePresence>
        {gameState === "PLAYING" && !isPortrait && (
          <motion.div
            initial={{ y: -80 }}
            animate={{ y: 0 }}
            className="absolute top-14 left-8 right-8 z-40 flex justify-between items-center pointer-events-none pt-[env(safe-area-inset-top)]"
          >
            ...
            <div className="bg-slate-900/90 backdrop-blur-md px-7 py-2.5 rounded-2xl text-white flex items-center gap-4 border border-white/10 shadow-2xl pointer-events-auto">
              <Timer
                className={`w-5 h-5 ${timeLeft < 30 ? "text-rose-400 animate-pulse" : "text-indigo-400"}`}
              />
              <p className="text-lg font-mono font-black tabular-nums">
          ...
          <header className="flex justify-between items-center mb-6 max-w-5xl mx-auto sticky top-0 bg-[#020617]/95 backdrop-blur-md py-4 z-10 px-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`p-2.5 bg-rose-600/20 text-rose-400 border border-rose-500/30 rounded-xl hover:bg-rose-600/40 transition-all ${isLoadingResults ? "opacity-30 cursor-not-allowed" : "active:scale-90"}`}
              ...
              <button
                ...
                className="px-5 py-2.5 bg-white/5 border border-white/10 rounded-xl font-bold flex items-center gap-2 hover:bg-white/10 text-xs active:scale-95 transition-all"
              >
              ...
              <button
                ...
                disabled={isLoadingResults}
                className={`p-2.5 bg-indigo-600 rounded-xl hover:bg-indigo-500 transition-all ${isLoadingResults ? "opacity-50 cursor-not-allowed" : "active:scale-90"}`}
              >
                {isLoadingResults ? (
                  <Loader2 size={18} className="animate-spin" />
                ) : (
